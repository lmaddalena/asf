...
mainmenu:	la $a0, msgmenu 	# mette in $a0 l'indirizzo della stringa msgmenu
		...
		# --
choice:		
		la $a0, msgchoice		# mette in $a0 l'indirizzo della strina msgmenu
		...
		j choice				# non � stato inserito una valore valido, ripete la richiesta di scegliere una opzione
...
opt1:		jal insgramm		# chiama la procedura insgramm
		...
		# --
		# -- se non � stata inserita la grammatica torna al menu principale
		# --
		lw $t0, TESTA		  		# $t0 = TESTA 
		beq $t0, $zero, mainmenu	# se $t0 = $zero non � stata inserita la grammatica, salta a meinmenu
		...
		# --
		# -- la grammatica inserita � corretta. Stampa il messaggio e torna al menu
		# --	
		la $a0, msgline 			# mette in $a0 l'indirizzo della stringa msgline
		li $v0, 4      				# codice per la stampa di una stringa
		syscall 					# stampa la linea di separazione
...
checkerr:	
		...
		# --
opt2:       	
		jal riconoscimento			# chiama la procedura riconoscimento
		j mainmenu					# torna all'inizio
...
msgzetaerr:	.asciiz "\n\n**** non � consentito iserire il simbolo Z poich� � il simbolo finale predefinito.\n"
instr:		.space 3				# riserva 3 byte per la stringa di input"
		...
		# --		
inssymb:	
		la $a0, msgsymb 			# mette in $a0 l'indirizzo della stringa msgsymb
		li $v0, 4      				# codice per la stampa di una stringa
		...
		# --					
		beq $s0, $t4, exitopt1		# se $s0 = $t4 (� stato premuto invio) salta a exitopt1 
		beq $s0, $t2, zetaerr		# se $s0 = $t2 (� stato inserito il carattere Z) salta a zetaerr
		blt $s0, $t3, symberr		# se $s0 < $t3 (� stato inserito un carattere non valido) salta a symberr
		bgt $s0, $t2, symberr		# se $s0 > $t2 (� stato inserito un carattere non valido) salta a symberr
		...
		# --							
symberr:	
		la $a0, msgparamerr			# mette in $a0 l'indirizzo della stringa msgparamerr
		li $v0, 4      				# codice per la stampa di una stringa	
		...
		# --										
zetaerr:	
		la $a0, msgzetaerr			# mette in $a0 l'indirizzo della stringa msgzetaerr
		li $v0, 4      				# codice per la stampa di una stringa	
		...
		# --
		# -- Il simbolo inserito � valido. Richiede di inserire le produzioni
		# --					
insprod:	
		la $a0, msgprod 			# mette in $a0 l'indirizzo della stringa msgprod
		li $v0, 4      				# codice per la stampa di una stringa	
		...
		la $a0, instr				# $a0 = puntatore all'area di memoria che conterr� la stringa
		...
		# --
		# -- Se � stato premuto invio sulla console torna a inserimento simbolo
		# --					
		li  $t0,  10				# $t0 =  10 (codice ascii Line Feed)
		beq $s1, $t0, inssymb		# se � stato premuto invio salta a insysymb (inserimento simbolo non terminale)


		# --
		# -- Effettua il parsing della produzione inserita. Se c'� un errore ripete l'inserimento
		# --					
		...
		beq $v0, $zero, insprod 	# se c'� stato un errore nel parsing chiede nuovamente di inserire la produzione

		# --
		# -- Il parsing � andata a buon fine. Salva i valori di ritorno della procedura insprod in $s1 e $s2 
		# --					
...
#                                                                                       #
# Valori di ritorno:                                                                    #
# $v0 - primo elemento della produzione dopo il parse (zero se c'� stato un errore)     #
# $v1 - secondo elemento della produzione dopo il parse (zero se c'� stato un errore)   #
#=======================================================================================#
parseprod:
		.data
msgerr1: 	.asciiz "\n\n**** valore inserito non valido\n"
msgerr2:	.asciiz "\n\n**** non � consentito iserire il simbolo Z poich� � il simbolo finale predefinito.\n"
		...
		# --
		blt  $s0, $t2, proderr		# se $s0 < $t2 (� stato inserito un carattere non valido) salta a proderr
		bgt  $s0, $t3, proderr		# se $s0 > $t3 (� stato inserito un carattere non valido) salta a proderr

		# --
		# -- Controlla se � stato inserito solamente il simbolo terminale
		# --	
		beq  $s1, $t4, nosymb 		# � stato inserito un solo carattere nella produzione, salta a nosymb

		# --
		# -- Controlla se � stato inserito il simbolo non terminale Z
		# --	
		beq  $s1, $t1, zerr			# se $s1 = $t1 (� stato inserito Z nel secondo carattere della produzione) salta a zerr				
	
		# --
		# -- Controlla che il secondo carattere della produzione sia comprezo tra A - Z
		# --
		blt  $s1, $t0, proderr		# se $s1 < $t0 (� stato inserito un carattere non valido) salta a proderr
		bgt  $s1, $t1, proderr		# se $s1 > $t1 (� stato inserito un carattere non valido) salta a proderr
		...
		# --
nosymb:		
		move $s1, $t1				# carica il codice ASCII del carattere Z nel registro $s1				
		move $v0, $s0				# valorizza il primo valore di ritorno
		move $v1, $s1				# valorizza il secondo valore di ritorno
		j    exitparseprod			# Salta a exitparseprod
		...
		# --
proderr:	
		la $a0, msgerr1				# mette in $a0 il puntatotr alla stringa msgerr1			
		li $v0, 4      				# codice per la stampa di una stringa		
		...
		# --
zerr:		
		la $a0, msgerr2 			# mette in $a0 il puntatotr alla stringa msgerr2					
		li $v0, 4      				# codice per la stampa di una stringa			
		syscall 					# stampa il messaggio di errore inserimento simbolo Z
...
addprod:
		...
		# --
		# -- Se la variabile TESTA � zero, � la prima allocazione di memoria
		# --
		lw $t1, TESTA		 		# carica in $t1 il valore contenuto nella variabile TESTA
		bne $t1, $zero, nonprimo 	# se non � il primo elemento salta a nonprimo
		
primo:		
		# --
		# -- Alla prima invocazione TESTA = CODA
		# --
		sw $v0, TESTA				# � il primo elemento, memorizza in TESTA l'indirizzo di partenza
		sw $v0, CODA				# � il primo elemento, CODA = TESTA
		j store
...
#=======================================================================================#
# controlla se la grammatica inserita � corretta                                        #
#                                                                                       #
# valoro di ritorno:                                                                    #
...
msgcheckerr:	.asciiz " --> **** Errore: produzione mancante"
		...
		beq $t0, $zero, exitCheck 	# esce se la lista non � stata inizializzata
...
loop1:		
		...
		lw $t1, CHR_Z
		beq $a0, $t1, next      	# se lultimo elemento della tripla � Z passa alla successiva
		...
		lw  $t1, TESTA			
loop2:		
		lw  $t2, 0($t1)				# carica in $t2 il simbolo non terinale (primo elemento della tripla)
		
		beq $a0, $t2, next      	# se $a0=$t2 la produzione � stata trovata passa al successivo elemento


		lw  $t1, 12($t1)			# puntatore all'elemento successivo			
		bne $t1, $zero, loop2		# itera fino a quando non trova nil (0)
		
		li  $t3, 1					# non � stata trovata la produzione. Alza il flag di errore
...
loop3:		lw  $t1, 0($t0)			# carica in $t1 il primo elemento della tripla
		...
		# --			
		bne $s0, $t1, next3			# il primo carattere � diverso da quello passato, passa alla successiva tripla
		bne $s1, $t2, next3			# il secondo carattere � diverso da quello passato, passa alla successiva tripla
		...
		bne $t4, $zero, _nonprimo	# se non � il primo elemento salta a nonprimo
		
_primo:		
		move $t4, $v0				# � il primo elemento, memorizza in $t4 l'indirizzo di partenza (testo)
		move $t5, $v0				# � il primo elemento, $t4 = $t5 (coda = testa)
		j _store
...
_store:		
		sw $t3, 0($v0)				# salva il valore inserito nei primi 4 byte
		sw $zero, 4($v0)			# salva nil = 0 nei restanti 4 byte	
	

next3:		
		lw  $t0, 12($t0)			# puntatore all'elemento successivo			
...
parola:		.space 64		#64 byte per la string di input (compresi i caratteri \n\0 finali)
		...
		# --
		# -- controlla se � stata inserita la grammatica
		# -- 
		lw  $t0, TESTA				# $t0 = TESTA
		bne $t0, $zero, insword		# se TESTA != 0, la  grammatica � stata inserita, prosegue.
		
		# --
		# -- non � stata inserita la grammatica. Stampa il messaggio ed esce
		# --		
		la $a0, msgnogramm 			# metto nel registro $a0 il puntatore alla stringa msgnogramm
		li $v0, 4      				# codice di stampa per le stringhe di caratteri
		syscall 					# stampa la stringa
		j exitriconoscimento		# esce dalla procedura poich� non � stata inserita alcuna grammatica
		...
		# -- 
insword:	
		la $a0, msginsert 			# metto nel registro $a0 il puntatore alla stringa msginsert
		li $v0, 4      				# codice di stampa per le stringhe di caratteri
		syscall 					# stampa la stringa
...
loopChar:		
		...
		beq $s4, $t3, nextChar		# se il carattere � uno spazio passa al successivo		


		# --
		# -- questa porzione di codice viene eseguita solo per il primo carattere della parola, ovvero se $s1 = $zero
		# --
		bne  $s1, $zero, np	  		# non � il primo carattere della parola, salta a nonprimo
...
loopStato:		
		...
		# -- 
		bne  $t1, $zero, np2	    # se la lista temporanea � vuota (primo elemento) salta a np2
...
nextStato:	
		...
		beq  $t1, $zero, riconerr 	# la lista degli stati � vuota, esce con errore
		...
		# --	
nextChar:	
		addi $s3, $s3, 1			# incrementa il puntatore della stringa
		lb   $s4, 0($s3)			# carica in $s4 il carattere successivo
		lw   $t3, CHR_LF			# carica in $t3 il codice ascii line_feed
		bne  $s4, $t3, loopChar    	# se non � stata raggiunta la fine della stringa itera
						

		# --
		# -- E' stata valutata tutta la parola, se la lista � vuota esce con errore
		# --
		beq  $s1, $zero, riconerr 	# la lista degli stati � vuota, esce con errore
...
loopCheck:		
		...
		beq $t1, $t2, riconok		# ha trovato il simbolo Z. La parola � stata riconosciuta
